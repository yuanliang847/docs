...
It is very likely that `php` does not support the currently selected architecture (`windows-amd64`).
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp winamd64/php:7.0-cli php your-script.php
```
...
```console
$ docker run -d -p 80:80 --name my-apache-php-app -v "$PWD":/var/www/html winamd64/php:7.0-apache
```

...
It is very likely that `postgres` does not support the currently selected architecture (`windows-amd64`).
...
```console
$ docker run -it --rm --link some-postgres:postgres winamd64/postgres psql -h postgres -U postgres
psql (9.5.0)
Type "help" for help.

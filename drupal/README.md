...
It is very likely that `drupal` does not support the currently selected architecture (`windows-amd64`).
...
```console
$ docker run --name some-drupal --link some-mysql:mysql -d winamd64/drupal
```
...
```console
$ docker run --name some-drupal --link some-postgres:postgres -d winamd64/drupal
```
